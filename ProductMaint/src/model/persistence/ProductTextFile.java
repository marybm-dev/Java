...
import static model.ProductConstants.CODE_SIZE;
import static model.ProductConstants.DESCRIPTION_SIZE;
...
public class ProductTextFile implements ProductDAO
{
    ...
    protected String padWithSpaces(String s, int length)
    {
        if (s.length() < length)
        {
    ...
    @Override
    public boolean notDuplicate(Product p) {
        Product temp = null;
        Boolean result = null;
        for (ListIterator itr = products.listIterator() ; itr.hasNext() ;) {
            temp = (Product) itr.next();
            result = temp.getCode().equals(p.getCode());
            if (result)
                return false;
        }
        return true;
